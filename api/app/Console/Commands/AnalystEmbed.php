...
class AnalystEmbed extends Command
{
    ...
    public function handle(SemanticRetriever $retriever, BuildMatchCardAction $card): int
    {
        ...
        $count = 0;
        $query->with('playerStats')->chunk(100, function ($matches) use ($retriever, $card, &$count) {
        ...
        $this->info("Embedded {$count} match card(s).");
