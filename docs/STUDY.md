...
- 22. [Local model or hosted](#22--local-model-or-hosted) — *AI, cost vs. quality*
...
## 21 · Splitting the store: ownership over convenience

*data ownership, the event handoff*

**The shared database topic 08 kept "for now" gets split along an ownership line — and the worker's direct UPDATE to a table it didn't own becomes an event to the table's owner.**
...
- The pattern then scaled twice — to rounds, then to the project's own docs. Three corpora, one embedder, and each unit answers what the others cannot: a match card finds the game, a round card the situation ("clutches we lost"), a doc chunk the rationale ("why is the parse queue a plain Redis list?") — the last a question grep cannot answer at all.
- The seams were claimed, then tested: swapping the placeholder embedder for nomic-embed-text, and Claude for a local model, was two classes, two match arms and a re-embed — no change to the action, the retriever, or one test of the RAG loop. A contract you have substituted behind beats one you assert is swappable.
...
- The projection is rebuilt on every swap, because vectors from different models are mutually meaningless: change the embedder and the column width changes (256 → 768), the migration wipes it, analyst:embed rebuilds. Disposable-by-design is what makes that safe rather than frightening.
...
- Three corpora produced three retriever contracts, not one. Matches and rounds scope to what the caller may see; documentation belongs to no match and no user, so a shared `corpus` parameter would have carried a scope argument meaningless to a third of its callers. The seam that did generalize was underneath — EmbeddingClient never changed.
- Retrieved evidence is a budget: every chunk lengthens the prompt, and generation is the slow half. The docs loop retrieves four rather than six for a measured reason — topic 22.

Every RAG project starts at "how do I use an LLM here", but the useful reframing is "what do I retrieve" — and Clutchlab had answered most of that before the word came up: the search read model (topic 04) and Postgres already held the evidence. Generation was the small new part, behind an interface, grounded so every claim cites a real [match:N]. Keyword and vector search are not rivals but complements from one source of truth, failing in opposite directions.

The docs corpus is where the abstraction was tested. Two similar corpora prove nothing; the third — scoped by nothing, identical for every caller — is what shows whether a seam generalizes or merely bends. It refused to fit, and the refusal was the result: it drew the line between the reusable part (embedding text, scanning vectors) and the part that only looked reusable (who may see what). A corpus is also defined by its exclusions — vendored docs outweighed the hand-written ones 3:2, and unfiltered, Laravel's stock Code of Conduct won retrieval slots from the architecture.
...
## 22 · Local model or hosted

*AI, cost vs. quality*

**The same interface can be served by a metered API or a container on your desk — and running both makes the tradeoff measurable instead of theoretical.**
...
- Whether it obeys a rule depends on the rule's FORM, not its importance. Asked to cite sources as [doc:path#heading] it produced zero citations, at every prompt length tried; asked for [1] it complied immediately, and the numbers are expanded back into paths server-side. Equally mandatory both times — only the difficulty changed. Design the output around what the model can emit, then translate.
- The dangerous failure is the fluent one. Asked why the parse queue is a plain Redis list, it returned seven confident, generically true advantages of Redis and never reached the real reason — Laravel's queue serializes PHP objects Go cannot read. Nothing about that answer looks broken; failing by invention reads exactly like succeeding.
...
- Free is not free: ~5GB of weights, VRAM competing with everything else on the machine, and a 600s timeout to survive the bad case. The hosted bill buys away a whole class of operational concern.

The interesting part is not that a local model works — it is that the project could find out cheaply. AnalystLlm and EmbeddingClient existed before either implementation did, so "how much worse is local?" cost two classes and no change to the RAG loop, and the answer arrived as evidence rather than opinion. That is the argument for interfaces stated precisely: not that you might replace a dependency someday, but that you can run the alternative today and compare.

The lever is rarely the prompt. When the docs answers came back as generic essays, two prompt rewrites changed nothing — cutting the evidence from six excerpts to four fixed it completely, on the same question, retrieval and similarity score. Past some volume a small model stops treating excerpts as the source and treats them as background, falling back on what it already knows. More retrieval is not more grounding, and the failure is invisible upstream: the scores looked healthy through every bad answer, so grounding can only be judged on the output.
